...
// Fallback component for loading with scroll-based animation
// function LoadingModel({ position, rotationSpeed = 1, scale = 1, scrollOffset = 0, color = "#666666" }) {
//   const meshRef = useRef();
//   const scroll = useScroll();
...
//       // Rotation based on scroll only
//       const rotationAmount = currentScrollProgress * Math.PI * 2 * rotationSpeed;
//       meshRef.current.rotation.x = rotationAmount * 0.5;
//       meshRef.current.rotation.y = rotationAmount * 0.8;
//       meshRef.current.rotation.z = rotationAmount * 0.3;
      
//       // Parallax movement from bottom to completely off-screen at top
//       const parallaxOffset = currentScrollProgress * 20; // Increased to 20 units for complete movement
//       meshRef.current.position.y = position[1] - 4 + parallaxOffset; // Start 4 units below, move 16 units above
      
//       // Horizontal position remains same
//       meshRef.current.position.x = position[0];
      
//       // Scale effect (slight change)
//       const scaleMultiplier = 1 + currentScrollProgress * 0.2;
//       meshRef.current.scale.setScalar(scale * scaleMultiplier);
...
//   return (
//     <mesh ref={meshRef} position={[position[0], position[1] - 4, position[2]]}>
...
export default function ParallaxScroll3D() {
  ...
  return (
    <div className="relative">
      {/* Pink Section */}
      {/* <section className="w-full h-screen bg-pink-300 flex items-center justify-center">
        <h1 className="text-6xl font-bold text-white">Scroll Down for 3D Magic</h1>
      </section> */}
      ...
      {/* <section className="w-full h-screen bg-blue-300 flex items-center justify-center">
        <h1 className="text-6xl font-bold text-white">End of 3D Journey</h1>
      </section> */}
    </div>
  );
}